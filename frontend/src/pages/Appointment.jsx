...
const Appointment = () => {
  ...
  const getClassName = (time) => {
    return `text-sm font-light flex-shrink-0 px-5 py-2 rounded cursor-pointer ${time === slotTime ? 'bg-primary text-white' : 'text-gray-400 border border-gray-300' }`
  }
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-auto mt-4'>
          {docSlots.length > 0 && docSlots[slotIndex].map((item, index)=>(
            <p onClick={()=>setSlotTime(item.time)} className={getClassName(item.time)} key={index}>
